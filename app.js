...
function saveAlarm() {
    var hour = document.getElementById("set-hr").value;
    var minute = document.getElementById("set-min").value;
    var second = document.getElementById("set-sec").value;
    document.getElementById("set-button").setAttribute("onclick", "cancelBtn()");

    function x() {
        document.getElementById("set-time").innerHTML = "Your alarm is set for " + `<strong class="horas">${hour}</strong>` + ":" + `<strong class="minutos">${minute}</strong>` + ":" + `<strong class="segundos">${second}<strong>` + ` <strong class="hs">hs</strong>`;
        ...
        document.getElementById("set-time").style.fontFamily = "Gill Sans, sans-serif"
        ...
        if(document.getElementById("hr").innerHTML == hour && document.getElementById("min").innerHTML == minute && document.getElementById("sec").innerHTML == second) {
            var audio = new Audio("mp3/Morning_mood.mp3");
            setInterval(() => {
                audio.play();
            }, 1000);
        } else {
